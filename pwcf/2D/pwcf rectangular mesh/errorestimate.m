function [e,uh] = errorestimate(u,n,stage)
uh = zeros(4,1);
e = 0;
for j = 1:(n-1)
    for i = 1:n
        ...
        fun = @(x1,x2) (pi.*cos(pi.*x1).*sin(pi.*x2)+uh(1)).^2+(pi.*sin(pi.*x1).*cos(pi.*x2)+uh(2)).^2;
        x1min = 1-j/n; x1max = 1-(j-1)/n; x2min = 1-i/n; x2max = @(x1) -x1+2-(i+j-1)/n;
        e = e + quad2d(fun,x1min,x1max,x2min,x2max);
        fun = @(x1,x2) (pi.*cos(pi.*x1).*sin(pi.*x2)+uh(3)).^2+(pi.*sin(pi.*x1).*cos(pi.*x2)+uh(4)).^2;
...
if stage == 0
   ...
   for s = 1:stage % local refinement
       ...
       fun = @(x1,x2) (pi.*cos(pi.*x1).*sin(pi.*x2)+uh(1)).^2+(pi.*sin(pi.*x1).*cos(pi.*x2)+uh(4)).^2;
       x1min = 1/(n*2^s); x1max = 1/(n*2^(s-1)); x2min = @(x1) x1; x2max = 1/(n*2^(s-1));
       e = e + quad2d(fun,x1min,x1max,x2min,x2max);
       fun = @(x1,x2) (pi.*cos(pi.*x1).*sin(pi.*x2)+uh(3)).^2+(pi.*sin(pi.*x1).*cos(pi.*x2)+uh(2)).^2;
