function [u,p,lambda] = pwcflargestage4(n,stage)
% f = -4, g = x1^2 + x2^2;
h = 1/n;
dimM = 4*(n*n+3*stage);
dimB = n*n+3*stage;
...
if stage ~= 0
    F(n*n) = 0;
    for s = 1:stage
        F(n*n+3*(s-1)) = 4*h*h/4^s;
        F(n*n+3*(s-1)+1) = 4*h*h/4^s;
        F(n*n+3*(s-1)+2) = 4*h*h/4^s;
    end
    F(dimB) = 4*h*h/4^stage;
end
% generating G
G = zeros(dimM,1);
for i = 1:n
    G(4*i) = h + ((1-(i-1)/n)^3-(1-i/n)^3)/3; % the line x1=1
    G(3+(i-1)*4*n) = h + ((1-(i-1)/n)^3-(1-i/n)^3)/3; % the line x2=1
end
for i = 1:n
    G(4*n*(n-1)+4*i-2) = -((1-(i-1)/n)^3-(1-i/n)^3)/3; % the line x1=0
    G(4*i*n-3) = -((1-(i-1)/n)^3-(1-i/n)^3)/3; % the line x2=0 
end
if stage ~= 0
    G(4*n*n-2) = 0;
    G(4*n*n-3) = 0;
for s = 1:stage
    G(4*(n*n-1+3*(s-1))+5) = -((h/(2^(s-1)))^3-(h/(2^s))^3)/3;
    G(4*(n*n-1+3*(s-1))+10) = -((h/(2^(s-1)))^3-(h/(2^s))^3)/3;
end
G(dimM-2) = -((h/(2^stage))^3)/3;
G(dimM-3) = -((h/(2^stage))^3)/3;
end
