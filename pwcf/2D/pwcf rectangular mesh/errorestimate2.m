function [e,uh] = errorestimate2(u,n,stage)
% g = x1.x2, f = 0
uh = zeros(4,1);
e = 0;
for j = 1:(n-1)
...
if stage == 0
   ...
   for s = 1:stage % local refinement
       ...
       fun = @(x1,x2) (x2+uh(1)).^2+(x1+uh(4)).^2;
       x1min = 1/(n*2^s); x1max = 1/(n*2^(s-1)); x2min = @(x1) x1; x2max = 1/(n*2^(s-1));
       e = e + quad2d(fun,x1min,x1max,x2min,x2max);
       fun = @(x1,x2) (x2+uh(3)).^2+(x1+uh(2)).^2;
